...
/*
 * E2E tests: P-DID x route-refresh and EoR events
 *
 * Prefix range: 30.x.0.0/16 (1-19)
 *
 * Tests:
 *   P-DID x E-ROUTE-REFRESH -- route refresh during detached init dump
 *   P-DID x E-EOR -- N/A (EoR already processed)
 */
...
/*
 * P-DID x E-EOR
 * N/A: EoR was already processed during the original INIT→JOINED_RUNNING
 * transition. Sending EoR to a peer in DETACHED_INIT_DUMP should be a
 * harmless no-op. Verify no crash and peer4 remains functional.
 */
TEST_P(UpdateGroupMultiPeerTest, DetachedInitDump_EorNoop) {
  XLOG(INFO, "=== TEST: DetachedInitDump_EorNoop ===");
  ...
  /* N/A: send EoR to peer3 during DID -- should be harmless no-op */
  sendEoRToPeer(peerId3);

  /* Verify group processes normally after EoR to DID peer */
  ...
  XLOG(INFO, "=== TEST PASSED: DetachedInitDump_EorNoop ===");
}

...
/*
 * E2E tests: P-DRJ x route-add, block, and slow-dur events
 *
 * Prefix range: 30.x.0.0/16 (20-49)
 *
 * Tests:
 *   P-DRJ x E-ROUTE-ADD -- route added during recovery
...
/*
 * P-DRJ x E-BLOCK
 * N/A: at DRJ the peer is already unblocked (that's how it reached DRJ).
 * Blocking a detached peer during recovery should be safe -- the group
 * continues normally with peer4. Verify no crash.
 */
TEST_P(UpdateGroupMultiPeerTest, DetachedReadyToJoin_BlockNoop) {
  XLOG(INFO, "=== TEST: DetachedReadyToJoin_BlockNoop ===");
  ...
  /* Unblock peer3 -- confirm DRJ before re-blocking */
  waitForDrjWithDrain(kPeerAddr3, peerId3);

  /* N/A: block peer3 again during recovery -- detached peer, safe no-op */
  blockPeer(kPeerAddr3);

  /* Inject a route to verify group continues normally */
  ...
  /* Cleanup: unblock peer3 for clean teardown */
  unblockPeer(kPeerAddr3);

  XLOG(INFO, "=== TEST PASSED: DetachedReadyToJoin_BlockNoop ===");
}

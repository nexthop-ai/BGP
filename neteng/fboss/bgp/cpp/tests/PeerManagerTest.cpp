...
TEST_F(PeerManagerTestFixture, SetRouteFilterPolicyScopeDeviceRegexTest) {
  ...
  auto& messages = subscribeToLogMessages(
      "neteng.fboss.bgp.cpp.peer.PeerManager", folly::LogLevel::INFO);

  // DEVICE_REGEX is the default key type (matchAgainstPeerGroupName() = false)
  rib_policy::TRouteFilterPolicy tPolicy;
  tPolicy.statements()->emplace("adjRib1", createTRouteFilterStatement({}));
  tPolicy.version() = 1;
  peerMgr->setRouteFilterPolicy(std::make_unique<RouteFilterPolicy>(tPolicy));

  // Run EVB until all coroutines complete (single-threaded, no TSAN race)
  evb.loop();
  folly::coro::blockingWait(peerMgr->asyncScope_.cancelAndJoinAsync());

  bool foundScopePeer = false;
  for (const auto& [msg, cat] : messages) {
    auto msgStr = msg.getMessage();
    if (msgStr.find("scope PEER") != std::string::npos &&
        msgStr.find("PEER_GROUP") == std::string::npos) {
      foundScopePeer = true;
      break;
    }
  }
  EXPECT_TRUE(foundScopePeer);
  SUCCEED();
}
...
TEST_F(PeerManagerTestFixture, SetRouteFilterPolicyScopePeerGroupNameTest) {
  ...
  // PEER_GROUP_NAME key type → matchAgainstPeerGroupName() = true → PEER_GROUP
  rib_policy::TRouteFilterPolicy tPolicy;
  tPolicy.statements()->emplace(
      "PEERGROUP_RSW_FSW_V4", createTRouteFilterStatement({}));
  tPolicy.key_type() = rib_policy::KeyType::PEER_GROUP_NAME;
  tPolicy.version() = 1;
  peerMgr->setRouteFilterPolicy(std::make_unique<RouteFilterPolicy>(tPolicy));
  ...
  bool foundScopePeerGroup = false;
  for (const auto& [msg, cat] : messages) {
    if (msg.getMessage().find("scope PEER_GROUP") != std::string::npos) {
      foundScopePeerGroup = true;
      break;
    }
  }
  EXPECT_TRUE(foundScopePeerGroup);
  SUCCEED();
}
...
TEST_F(
    PeerManagerDynamicPolicyEvaluationFixture,
    UpdateIngressEgressPolicyNamesScopePeerTest) {
  SetUp(false /* enableDynamicPolicyEvaluation */);
  ...
  // Call with PolicyChangeScope::PEER (simulates setPeersPolicy path)
  peerMgr->updateIngressEgressPolicyNames(
      std::move(policyMap), PolicyChangeScope::PEER);
...
TEST_F(
  ...
  // Call with PolicyChangeScope::PEER_GROUP (simulates setPeerGroupsPolicy
  // path)
  peerMgr->updateIngressEgressPolicyNames(
      std::move(policyMap), PolicyChangeScope::PEER_GROUP);

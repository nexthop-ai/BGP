...
/* Return memory the process currently used from /proc/[pid]/status.
 ...
 / structures.
*/
std::optional<size_t> getMemBytes(const std::string& memoryType) {
...
std::optional<size_t> SystemMetrics::getRSSMemBytes() {
  return getMemBytes("VmRSS");
}

// Return virtual memory the process currently used
std::optional<size_t> SystemMetrics::getVirtualMemBytes() {
...
*/
std::optional<double> SystemMetrics::getCPUpercentage() {
  struct rusage usage;
...
uint64_t SystemMetrics::getCurrentNanoTime() {

 ...
 *
 * Vendored from github.com/facebook/openr/openr/common/NetworkUtil.cpp.
 * No content changes — only the include path resolves locally now.
 *
...
size_t hash<openr::thrift::IpPrefix>::operator()(
    openr::thrift::IpPrefix const& ipPrefix) const {
  return hash<string>()(*ipPrefix.prefixAddress()->addr()) +
      *ipPrefix.prefixLength();
}

size_t hash<openr::thrift::BinaryAddress>::operator()(
...
size_t hash<openr::thrift::MplsAction>::operator()(
...
size_t hash<openr::thrift::NextHopThrift>::operator()(
...
size_t hash<openr::thrift::UnicastRoute>::operator()(
